...
{%- for post in collections.blog | sortByPubDate | reverse | limit(4) -%}
  <li role="listitem">
    <stack-l>
      ...
      <p data-pagefind-weight="0.5">{{ post.data.excerpt | safe }}</p><!-- No line break here! -->{% include "_tags-buttons.njk" %}
      <hr />
    </stack-l>
  </li>
{%- endfor -%}
{%- for post in collections.blog | sortByPubDate | reverse | startFrom(4) -%}
  <li role="listitem">
    <stack-l>
      <h2 data-pagefind-weight="0.5"><a href="{{ post.url }}">{{ post.data.title | safe }}</a></h2>
      <a href="{{ post.url }}">
        ...
        %}
        <span class="sr-only">Read “{{ post.data.title | safe }}”</span>
      </a>
      <small>
        {% if post.data.pubdate %}
          <time datetime="{{ post.data.pubdate | toISOString | safe }}"><!-- No line break here! -->{{ post.data.pubdate | dateUK | safe }}</time>
        {% else %}
          No publication date found.
        {% endif %}
      </small>
      <p data-pagefind-weight="0.5">{{ post.data.excerpt | safe }}</p>{% include "_tags-buttons.njk" %}
      <hr />
    </stack-l>
  </li>
{%- endfor -%}
