...
export default class ThemeSwitcher extends HTMLElement {
  ...
  connectedCallback() {
    ...
    this.button.setAttribute(
      `title`,
      `${themeMessage} (Switch to ${themeValue} theme)`,
    );
  ...
  handleEvent() {
    ...
    if (themeAttr !== `dark`) {
      this.button.setAttribute(`data-theme`, `dark`);
      document.documentElement.setAttribute(`data-theme`, `dark`);
      localStorage.setItem(`theme-preference`, `dark`);
      themeVal.innerHTML = `light`;
      themeStatus.innerHTML = `dark`;
      this.button.setAttribute(`title`, `Dodge me! (Switch to light theme)`);
    } else {
      ...
      this.button.setAttribute(`title`, `Burn me! (Switch to dark theme)`);
    }
  }
  ...
  render() {
    ...
    const dataTheme = storageKey === `dark` ? `dark` : `light`;
...
if (`customElements` in window) {
  // let the browser know that <mpb-theme-toggle> is served by our new class
  customElements.define(`mpb-theme-toggle`, ThemeSwitcher);
}
